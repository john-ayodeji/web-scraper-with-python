(() => {
  ...
  // The key is stored only in the browser's native input slot (masked by
  // type="password").  The JS .value property is overridden to always return ""
  // so it cannot be read from the browser console or any external script.
  // Copy, cut, and context-menu events are blocked on the field.
  const _apiKeyInput = document.getElementById("resend_api_key");
  const _nativeValueDesc = Object.getOwnPropertyDescriptor(
    HTMLInputElement.prototype,
    "value",
  );

  Object.defineProperty(_apiKeyInput, "value", {
    get() {
      return "";
    },
    set(v) {
      _nativeValueDesc.set.call(_apiKeyInput, v ?? "");
    },
    configurable: false,
    enumerable: true,
  });

  _apiKeyInput.addEventListener("copy", (e) => e.preventDefault());
  _apiKeyInput.addEventListener("cut", (e) => e.preventDefault());
  _apiKeyInput.addEventListener("contextmenu", (e) => e.preventDefault());
  // ── End Resend API key protection ───────────────────────────────────────────
  ...
  function formPayload() {
    return {
      ...
      resend_api_key: _nativeValueDesc.get.call(_apiKeyInput),
  ...
  Promise.all([fetchSettings(), fetchStatus(), fetchReport()]).catch(
    (error) => {
      errorText.textContent = String(error.message || error);
    },
  );
